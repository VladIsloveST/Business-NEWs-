...
class HistoryTableView: UITableView {
    ...
    private func configure() {
        ...
        register(HistoryTableViewCell.self, forCellReuseIdentifier: HistoryTableViewCell.identifier)
...
extension HistoryTableView: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.didDelete = { [weak self] in
            ...
            self?.reloadData() }
        cell.didRevert = { [weak self] in
            self?.mainCellDelegate?.selectItem(row: indexPath.row, with: .revert)
        }
        cell.setup(search: searchingHistory[indexPath.row])
        return cell
    }
}
